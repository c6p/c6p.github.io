...
with open('skills.csv') as f:
  ...
  with open('skills.dot', 'w') as svg:
    svg.write(f'''digraph "skills" {{
      ...
      graph[fontname="BenchNine" rankdir=LR, center=true, margin=0, nodesep=0, ranksep=0]
      node[fontname="BenchNine" shape=none, margin=0.01]
      edge[fontname="BenchNine" arrowsize=0.6, arrowhead=vee color="#c9d1d9"]
      {nodes}
      {edges}
    }}''')
